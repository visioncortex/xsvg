...
| File | Shows |
|---|---|
| [aqua.xsvg](aqua.xsvg) | **The classic mesh use case**: macOS Aqua gel buttons in **two patches** — an opaque gel body (superelliptic rim vignette + nonlinear bottom underlight) under a **feathered gloss layer** (per-vertex `#rrggbbaa` alpha, world-map meridian warp, crisp lower edge from the mesh boundary), plus a compiled `drop-shadow()` |

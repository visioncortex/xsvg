...
| File | Shows |
|---|---|
| [aqua.xsvg](aqua.xsvg) | **The classic mesh use case**: macOS Aqua gel buttons in **two patches** — an opaque gel body (superelliptic rim vignette + nonlinear bottom underlight) under a **feathered gloss overlay** whose mesh *is* an inset capsule (60% of the button height, arc-angle end columns, per-vertex `#rrggbbaa` alpha dissolving ~60% down), plus a compiled `drop-shadow()` |

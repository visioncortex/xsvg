...
| **Envelope distort — warp presets** (arc, arch, bulge, flag, wave, fisheye, inflate, squeeze, twist, rise, shell, fish, stair) | AI | S | ✅ | **all 15 shipped** — `<x:warp>` (§7.3) bakes outlined glyphs + shapes through the kurbo pipeline; catalog in [Transform.md §B](Transform.md) |

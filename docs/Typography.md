...
| Type-on-path options: align (asc/desc/center/baseline), spacing, flip, effect (rainbow/skew/3D-ribbon/stair/gravity) | AI | S | ◑ | **skew + rainbow shipped** (`<x:textpath>`, §6.13) incl. `baseline-shift` along the normal; align/spacing/flip and the remaining effects still open |

...
**v0 column:** can the browser-`<text>` v0 ([PLAN](Plan.md) §2 Phase 2a) do it? ✅ yes (native
SVG/CSS) · ◑ partial / approximate · ❌ needs Phase 2b outlining or custom layout.

---

## A. Text containers & flow — *where text lives*

| Capability | From | Tier | v0 | xsvg note |
|---|---|---|---|---|
| **Point type** (unwrapped text at an anchor) | AI | C | ✅ | maps to SVG `<text x y>` |
| **Area type** — flow inside a **rectangle** | AI | C | ◑ | greedy wrap from `measureText`; emit positioned `<tspan>`s |
...
| **Type on a path** (baseline follows a curve) | AI | E | ◑ | SVG `<textPath>` exists (v0); align-to-path + spacing options need custom |
...
| **Vertical writing mode** (CJK top-to-bottom) | AI/PDF | S | ◑ | SVG `writing-mode`; real CJK rules are Stretch |
| Inset spacing / first-baseline offset (ascent, cap-height, leading, fixed) | AI | E | ◑ | container padding + baseline origin rule |

## B. Paragraph composition — *breaking, spacing, alignment*

| Capability | From | Tier | v0 | xsvg note |
|---|---|---|---|---|
| **Alignment**: left / right / center | AI | C | ✅ | `text-anchor` per line |
...
| **Leading / line spacing**: absolute + auto (% of size) | AI | C | ✅ | Vision item; line advance in layout |
| **Space before / after** paragraph | AI | C | ✅ | layout gaps |
...
| **Bidi / paragraph direction** (LTR/RTL) | AI/PDF | E | ◑ | browser does bidi in v0; deterministic in 2b (ICU) |
...
| **Variable-font axes** (wght/wdth/opsz/slnt + custom) | AI | E | ◑ | CSS `font-variation-settings` in v0; full control in 2b |
| **Tracking** (uniform range letter-spacing) | AI | C | ✅ | PDF `Tc`; SVG `letter-spacing` |
| **Word spacing** | AI | E | ✅ | PDF `Tw`; SVG `word-spacing` |
| **Kerning**: metrics / **optical** / manual pair | AI | E | ◑ | metrics free in v0; optical & manual need shaping (2b) |
| **Horizontal scale** (glyph width) | AI/PDF | C | ✅ | Vision "character width scaling"; PDF `Tz`; SVG transform/`textLength` |
| **Vertical scale** (glyph height) | AI/PDF | E | ◑ | non-uniform glyph scaling |
| **Baseline shift** (super/subscript, arbitrary) | AI/PDF | C | ✅ | PDF `Ts` text-rise; SVG `dy`/`baseline-shift` |
| **Per-glyph rotation** | AI/PDF | E | ◑ | SVG `rotate` list; needed for vertical text |
...
| **Underline / strikethrough** (+ weight/offset/type) | AI/ID | E | ◑ | basic via CSS; custom decoration in 2b |
| **OpenType — ligatures** (standard, discretionary) | AI | E | ◑ | browser applies `liga`; explicit control in 2b |
| **OpenType — contextual alternates, swashes, stylistic sets/alternates, titling** | AI | E | ◑ | `font-feature-settings` in v0; precise in 2b |
...
| **Language / locale tagging** (affects shaping, hyphenation, features) | AI/PDF | E | ◑ | `lang`/`xml:lang` |
...
| Capability | From | Tier | v0 | xsvg note |
|---|---|---|---|---|
| **Per-range fill** (solid) | AI/PDF | C | ✅ | Vision item; PDF render mode `Tr 0` |
| **Per-range stroke** (color, width, dash, join/cap) | AI/PDF | C | ◑ | Vision item; PDF `Tr 1`; SVG stroke on `<text>` (v0 basic) |
| **Fill + stroke together**, with paint order | AI/PDF | C | ◑ | PDF `Tr 2`; SVG `paint-order` |
| **Gradient fill** on text (linear/radial), block-wide or per-glyph | AI | E | ◑ | gradient `fill` (v0); per-glyph needs outlining |
...
| **Opacity / blend mode** per range or per fill | AI | E | ◑ | SVG opacity (v0); blend modes (`mix-blend-mode`) |
| **Effects**: drop shadow, outer/inner glow, blur | AI | E | ◑ | SVG filters; some work on `<text>` in v0 |
...
| **Invisible text** (present but not painted, for selection/extraction) | PDF | S | ✅ | PDF `Tr 3`; SVG opacity 0 / aria |
...
| **Char spacing `Tc`, word spacing `Tw`** | §9.3.2-3 | tracking / space adjust |
| **Horizontal scale `Tz`** | §9.3.4 | glyph width scaling |
...
| Capability | From | Tier | v0 | xsvg note |
|---|---|---|---|---|
| **Unicode round-trip** (text stays real text in source) | PDF `ToUnicode` | C | ✅ | xsvg source is the text; preserve in output where possible |
...
## What v0 (browser `<text>`) realistically delivers

**Yes in v0 (✅/◑):** point & rectangular area type, alignment, leading, space before/after,
indents, tracking/word-spacing, horizontal scale, baseline shift, named fonts + variable-font
settings, browser-applied OpenType features & ligatures & bidi & kerning (metrics), per-range solid
**fill**, basic per-range **stroke**, gradient fill, opacity, simple filters, selectable text. Greedy
line breaking and rectangular justification are doable from `measureText`.

**Needs Phase 2b (outlining + custom layout, ❌):** arbitrary-polygon flow, Knuth-Plass justification
with glyph-scaling limits, hyphenation, optical margin alignment, the **★ mesh-gradient fill** and
**★ variable-width stroke** on glyphs, pattern fills, envelope/warp distortion, precise/optical
kerning, true small caps, glyph-by-GID access, and deterministic cross-browser layout.
